...
export const Dashboard = ({ onViewChange }: DashboardProps) => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      // Combine project IDs
      const allProjectIds = [...(ownedProjects || []).map(p => p.id)];
      
      if (memberProjects) {
        memberProjects.forEach(membership => {
          if (membership.projects && !allProjectIds.includes(membership.projects.id)) {
            allProjectIds.push(membership.projects.id);
          }
        });
      }
      ...
      // Load recent activities (combining user stories and test cases updates)
  ...
  const statsConfig = [
    ...
    {
      title: "AI Usage",
      value: stats.aiUsageCount.toString(),
      change: "AI-powered features",
      icon: Brain,
      color: "text-warning"
    }
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        ...
        <Button variant="gradient" onClick={() => onViewChange('user-stories')}>
